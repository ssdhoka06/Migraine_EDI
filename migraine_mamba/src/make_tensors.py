...
class DataConfig:
    ...
    # Sequence parameters
    seq_len: int = 14  # 14-day lookback
    pred_horizon: int = 1  # Predict 1 day ahead
    ...
    # Split parameters
    test_size: float = 0.1  # 10% held out for testing
...
class MigraineDataProcessor:
    ...
    def _compute_clinical_features(
        ...
        if 'menstrual_cycle_day' in seq_data.columns:
            ...
            if last_cycle_day >= 0:  # Female
                high_risk = last_cycle_day in [0, 1, 26, 27]
                features[1] = float(high_risk)
        
        # 3. Attack count last 7 days
        last_7 = seq_data.tail(7)
        features[2] = last_7['attack'].sum() / 7.0  # Normalized
        
        # 4. Attack count last 14 days
        features[3] = extended_data['attack'].sum() / 14.0  # Normalized
        
        # 5. Trigger accumulation (low sleep + high stress in last 3 days)
        ...
        if 'stress_level' in last_7.columns and len(last_7) >= 2:
            ...
            features[7] = (second_half - first_half) / 5.0  # Normalized
    ...
    def create_sequences(
        ...
        Returns:
            ...
            targets: Attack labels for prediction
        """
        ...
        for patient_id in tqdm(patient_ids, desc="Processing patients"):
            ...
            # Need at least seq_len + pred_horizon days
    ...
    def normalize_features(
        ...
        # Reshape for scaler: (n_samples * seq_len, n_features)
        ...
        normalized[nan_mask] = 0  # Keep NaN positions as 0
    ...
    def split_data(
        ...
        def get_split(patients):
            ...
            split_features = {
                key: value[mask] if key != 'patient_ids' else value[mask]
                for key, value in features.items()
            }
            split_targets = targets[mask]
            return split_features, split_targets
        
        train_features, train_targets = get_split(train_patients)
        val_features, val_targets = get_split(val_patients)
        test_features, test_targets = get_split(test_patients)
        
        print(f"\nData split:")
        print(f"  Train: {len(train_targets):,} sequences ({len(train_patients):,} patients)")
        print(f"  Val:   {len(val_targets):,} sequences ({len(val_patients):,} patients)")
        print(f"  Test:  {len(test_targets):,} sequences ({len(test_patients):,} patients)")
    ...
    def process_and_save(
        ...
        """
        Complete pipeline: load, process, split, and save data.
        """
        # Load data
        ...
        # Normalize splits using fitted scaler
        train_f = self.normalize_features(train_f, fit=False)
        val_f = self.normalize_features(val_f, fit=False)
        test_f = self.normalize_features(test_f, fit=False)
        ...
        # Print class balance
        print(f"\nClass balance:")
        print(f"  Train: {train_t.mean():.2%} attacks")
        print(f"  Val:   {val_t.mean():.2%} attacks")
        print(f"  Test:  {test_t.mean():.2%} attacks")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data", "-d",
        default="data/migraine_synthetic_data.csv",
